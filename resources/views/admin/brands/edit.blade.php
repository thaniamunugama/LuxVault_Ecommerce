...
<div class="bg-white rounded-xl shadow-sm p-6 max-w-xl">
  <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    ...
    <div>
      ...
      <input type="text" name="name" value="{{ old('name', $brand->name) }}" class="mt-1 w-full border rounded px-3 py-2" required />
    </div>
    <div>
      ...
      @if($brand->image)
        <div class="mt-2">
          <img src="{{ asset('storage/' . $brand->image) }}" alt="{{ $brand->name }}" class="h-12 object-contain" />
        </div>
      @endif
    </div>
    <div class="pt-2">
